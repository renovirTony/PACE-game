...
export const EquipmentCardView: React.FC<EquipmentCardViewProps> = ({
  ...
  return (
    <div
      className={`relative flex flex-col justify-between rounded-2xl border transition-all duration-200 overflow-hidden w-full ${
    ...
    >
      <div>
        {/* Top Header: Slot Tag & Cost / Status */}
        <div className="flex items-center justify-between gap-2 mb-2.5 flex-wrap">
          <div className="flex items-center gap-1.5 flex-wrap">
            <span
              className={`px-2.5 py-0.5 text-xs font-black font-mono rounded-lg tracking-wider border ${
                slotTheme.border
              } ${slotTheme.text} bg-black/60 shadow-sm`}
            >
              [{card.slot}] {slotTheme.label.split(' ')[0]}
            </span>
            {card.bonusVP ? (
              <span className="px-2 py-0.5 text-xs font-bold rounded-lg bg-amber-500/20 text-amber-300 border border-amber-500/40 font-mono">
                +{card.bonusVP} VP 獎勵
              </span>
            ) : null}
          </div>
          ...
          {!isEquipped && (
            <div className="flex items-center gap-1 text-xs sm:text-sm font-mono font-bold text-amber-400 bg-amber-950/70 px-2.5 py-0.5 rounded-lg border border-amber-500/40 shrink-0 shadow-inner">
              <span className="text-slate-400 text-xs font-normal">採購:</span>
              <span>{card.cost}</span>
        ...
        <div className="flex items-start gap-2.5 mb-2.5">
          <div className={`p-2.5 rounded-xl bg-black/60 ${slotTheme.text} border border-white/10 shrink-0 shadow-inner`}>
        ...
        <div className="flex flex-wrap gap-1.5 mb-2.5 text-xs font-mono">
          <span className={`px-2 py-0.5 rounded-lg border font-semibold ${bw.color}`}>
            {bw.text}
          </span>
          <span className="px-2 py-0.5 rounded-lg border border-slate-700 bg-slate-900/90 text-slate-200">
            {RANGE_LABELS[card.range] || card.range}
          </span>
          <span className="px-2 py-0.5 rounded-lg border border-amber-500/30 bg-amber-950/50 text-amber-300 flex items-center gap-1 font-bold">
        ...
        {(card.resilience.empShield || card.resilience.weatherResistant || card.resilience.subterranean) && (
          <div className="flex flex-wrap gap-1.5 mb-2.5">
            {card.resilience.empShield && (
              <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-lg bg-cyan-950/90 text-cyan-300 border border-cyan-500/40 font-semibold">
                <Shield className="w-3 h-3 text-cyan-400" /> 抗EMP/電子戰
              </span>
            )}
            {card.resilience.weatherResistant && (
              <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-lg bg-blue-950/90 text-blue-300 border border-blue-500/40 font-semibold">
                <CloudLightning className="w-3 h-3 text-blue-400" /> 全天候防護
              </span>
            )}
            {card.resilience.subterranean && (
              <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-lg bg-emerald-950/90 text-emerald-300 border border-emerald-500/40 font-semibold">

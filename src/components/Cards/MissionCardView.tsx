    ...
    <div
      className={`relative flex flex-col justify-between rounded-xl border p-4 transition-all duration-200 overflow-hidden ${
        ...
        isClaimedByMe
          ...
          ? 'bg-gradient-to-br from-slate-900 via-[#0e1626] to-slate-900 border-cyan-500/50 shadow-lg shadow-cyan-950/40 hover:border-cyan-400'
    ...
    >
      <div>
        ...
        <div className="flex items-center justify-between gap-2 mb-3">
          <span className={`text-xs font-bold font-mono px-2.5 py-1 rounded border uppercase tracking-wider ${urgency.color}`}>
            {urgency.label}
          </span>

          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1 text-xs sm:text-sm font-bold text-amber-300 bg-amber-950/60 px-2.5 py-1 rounded border border-amber-500/40 font-mono">
              <Award className="w-4 h-4 text-amber-400" />
              {mission.vp} VP
            </span>
            <span className="text-xs sm:text-sm font-mono text-cyan-300 bg-cyan-950/60 px-2.5 py-1 rounded border border-cyan-500/40">
        ...
        <div className="flex items-start gap-3 mb-2.5">
          <div className="p-2.5 rounded-lg bg-black/40 text-cyan-400 border border-cyan-500/30 shrink-0">
            <IconRenderer name={mission.iconName} className="w-6 h-6" />
          </div>
          <div>
        ...
        <p className="text-xs sm:text-sm text-slate-300 leading-relaxed mb-3 bg-slate-950/50 p-2.5 rounded-lg border border-white/5">
          {mission.description}
        </p>
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          ...
          {mission.requiresEmpShield && (
            <span className="text-xs font-mono px-2 py-0.5 rounded bg-red-950 text-red-300 border border-red-500/40 font-semibold">
              ⚡ 需抗EMP
            </span>
          )}
          {mission.requiresWeatherResist && (
            <span className="text-xs font-mono px-2 py-0.5 rounded bg-blue-950 text-blue-300 border border-blue-500/40 font-semibold">
              🌧️ 需耐候
            </span>
          )}
          {mission.requiresSubterranean && (
            <span className="text-xs font-mono px-2 py-0.5 rounded bg-emerald-950 text-emerald-300 border border-emerald-500/40 font-semibold">
              🕳️ 需地底穿透
            </span>
          )}
          {mission.restrictedSlots && (
            <span className="text-xs font-mono px-2 py-0.5 rounded bg-red-950/90 text-red-300 border border-red-500/50 font-bold">
              🚫 禁用 [{mission.restrictedSlots.join(',')}] 槽位
            </span>
          )}
          {mission.minSlotRequirement && (
            <span className="text-xs font-mono px-2 py-0.5 rounded bg-amber-950/90 text-amber-200 border border-amber-500/50 font-bold">
        ...
        <div className="p-2.5 rounded-lg bg-black/50 border border-white/10 text-xs font-mono mb-3">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-slate-400">目前通訊連通狀態:</span>
            {transmissionEval.canTransmit ? (
              <span className="text-emerald-400 font-bold flex items-center gap-1">
                <CheckCircle2 className="w-4 h-4" /> 可連通 [{transmissionEval.successfulSlot}]
                {transmissionEval.bonusPoints > 0 && ` (+${transmissionEval.bonusPoints} VP)`}
              </span>
            ) : (
              <span className="text-red-400 font-bold flex items-center gap-1">
                <AlertCircle className="w-4 h-4" /> 無法連通
              </span>
            )}
          </div>
      ...
      {isClaimedByMe ? (
        <div className="py-2.5 px-3 rounded-lg bg-emerald-950/50 border border-emerald-500/40 text-emerald-300 text-xs sm:text-sm font-mono font-bold text-center flex items-center justify-center gap-2">
        ...
        <button
          ...
          className={`w-full py-2.5 px-3 rounded-lg text-xs sm:text-sm font-bold font-mono tracking-wider transition-all flex items-center justify-center gap-2 ${

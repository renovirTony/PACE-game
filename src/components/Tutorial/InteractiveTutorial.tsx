...
const TUTORIAL_STEPS: Record<number, StepContent> = {
  ...
  7: {
    title: '第七步：採購 [C] 槽位應急通訊設備',
    badge: '回合 2 · 步驟 7 / 9 · 深度防線',
    icon: <ShoppingBag className="w-6 h-6 text-amber-400" />,
    instruction: '在極端災害或強烈電磁干擾下，有線通訊是極為堅固的防線！請在左下方補給庫中，找到【野戰被覆線電話 (TA-312)】，點擊「裝備至 [C] 槽位」！',
    tips: '💡 [C] 應急槽位在無線電遭干擾時是至關重要的生命線！',
    actionRequired: '👉 請在下方點擊【裝備至 [C] 槽位 (消耗 1 AP)】'
  },
  8: {
    title: '第八步：發起第二項突發危機任務',
    badge: '回合 2 · 步驟 8 / 9 · 多元情境實戰',
    icon: <Radio className="w-6 h-6 text-blue-400" />,
    instruction: '請檢視左上方的危機任務（例如【高解析度空拍情資回傳】或【地下指揮坑道崩塌救援】），點擊【發起廣播連通】再次驗證您的通訊防線！',
    tips: '💡 觀察任務需求與自身裝備的頻寬、範圍與抗性，選擇最合適的任務進行連通。',
    actionRequired: '👉 請點擊左上方危機任務的【發起廣播連通 (消耗 1 AP)】'
  },
  9: {
    title: '第九步：結束第二回合',

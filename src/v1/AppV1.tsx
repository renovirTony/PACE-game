...
export function AppV1({ onSwitchToV2 }: AppV1Props) {
    ...
    const checkVersion = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (data.buildTime && typeof (window as any).__APP_BUILD_TIME__ !== 'undefined' && data.buildTime !== (window as any).__APP_BUILD_TIME__) {
  ...
  // Top Version Switcher Bar
  const renderVersionBanner = () => (
    <div className="w-full bg-slate-900/90 border-b border-cyan-500/30 px-4 py-2 flex items-center justify-between text-xs font-mono text-slate-300">
      <div className="flex items-center gap-2">
        <span className="px-2 py-0.5 rounded bg-cyan-950 text-cyan-400 border border-cyan-500/40 font-bold">
          v1.0 經典軍規原版
        </span>
        <span className="hidden sm:inline text-slate-400">
          固定 P/A/C/E 槽位 · 戰術應急模擬
        </span>
      </div>
      {onSwitchToV2 && (
        <button
          onClick={onSwitchToV2}
          className="flex items-center gap-1.5 px-3 py-1 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-400 hover:to-blue-400 text-slate-950 font-bold transition-all shadow-sm active:scale-95"
        >
          <Sparkles className="w-3.5 h-3.5" />
          <span>切換至全新重構版 v2.0 (民防自組/線上連線) ➔</span>
        </button>
      )}
    </div>
  );
  ...
  if (gameState.phase === 'SETUP') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#060913] text-slate-100 relative overflow-hidden">
        {renderVersionBanner()}
        ...
        <div className="relative z-10 max-w-5xl w-full my-auto p-4 sm:p-8">
          <div className="rounded-3xl border border-cyan-500/30 bg-slate-950/90 p-6 sm:p-10 shadow-2xl backdrop-blur-2xl">
